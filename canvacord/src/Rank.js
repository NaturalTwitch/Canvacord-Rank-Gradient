...
class Rank {
   ...
   * @param {"COLOR"|"IMAGE"|"GRADIENT"} type Background type
   * @param {string|Buffer|Array} [data] Background color or image
   * @returns {Rank}
   */
  setBackground(type, data) {
    ...
    switch (type) {
      ...
      case "IMAGE":
        ...
        break;
      case "GRADIENT":
        this.data.background.type = "gradient";
        this.data.background.image = `gradient:${data}`;
        break;
      default:
  ...
  async build(
    ...
    if (!!bg) {
      ...
      if (this.data.background.image.startsWith("gradient:")) {
        // Parse the colors from the background string
        let colors = this.data.background.image.slice(9).split(",");
    ...
    if (this.data.level.display && !isNaN(this.data.level.data)) {
        ...
        const levelX = 850 - levelTextWidth; // adjust x-coordinate for level text
       
        ctx.fillText(levelText, levelX, 82);
       
        ctx.font = `bold 32px ${ops.fontX}`;
        ctx.fillStyle = this.data.level.color;
        ctx.textAlign = "start"; // Ensure text alignment is set to start
       
        const levelNumber = Util.toAbbrev(parseInt(this.data.level.data));
        const levelNumberWidth = ctx.measureText(levelNumber).width;
        const spacing = 10; // adjust the spacing value as needed
       
        // Calculate the x-coordinate for the level number
        // This ensures the level number is drawn to the right of the "LEVEL" text
        const levelNumberX = levelX + levelTextWidth + spacing;
       
        // Draw the level number
        ctx.fillText(levelNumber, levelNumberX, 82);
       }
